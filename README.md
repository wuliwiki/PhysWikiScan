...
* 使用 `--history` 把 `../PhysWiki-backup/*.tex` 备份文件信息更新到数据库。 更新数据库中的词条作者列表。
* 使用 `--fix-db` 重新生成标记为【生成】的数据（不包括作者）。
...
## Plan: Use Sqlite
* 按照 `./PhysWikiScan .` 的执行顺序开始修改， 然后再实现 `--toc`、 `--entry`、 `--autoref` 等。
* 目标是每次用 `--entry` 都更新所有必要的数据， 最后不需要使用 `./PhysWikiScan .` 命令。
* 为保持稳定， 在写入数据文件的基础上另外同步写入数据库文件。
* 每次更新把少量文件读取命令改成数据库读取命令， 然后进行测试。
* 直到一个文件不再需要读取， 就可以把写入该文件的命令删掉。
