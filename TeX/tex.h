...
// replace with `PhysWikiScanHideVerb`, `PhysWikiScanHideLstinline`, `PhysWikiScanHideLstlisting`
inline Long verbatim_o(vecStr32_O str_verb, Str32_IO str)
{
    Long ind0 = 0, ind1, ind2;
    Char32 dlm;
    Str32 tmp;
    ...
    while (true) {
        ...
        str.replace(ind0, ind2 + 1 - ind0, U"PhysWikiScanHideVerb");
        ind0 += 3;
    }
    ...
    while (true) {
        ...
        str.replace(ind0, ind2+1 - ind0, U"PhysWikiScanHideLstinline");
        ind0 += 3;
    }
    ...
    for (Long i = N - 1; i >= 0; --i) {
        ...
        str.replace(intv.L(i), intv.R(i)+1 - intv.L(i), U"PhysWikiScanHideLstlisting");
    }
    for (Long i = 0; i < N; ++i) {
