...
inline Long issuesEnv(Str_IO str)
{
	...
	if (ind0 > 0) {
		...
		str.replace(ind0, ind1 - ind0, u8"<li>本文处于草稿阶段。</li>");
		++N;
	}
	// issueTODO
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li>本文存在未完成的内容。</li>");
		++N;
	}
	// issueMissDepend
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li>本文缺少预备知识， 初学者可能会遇到困难。</li>");
		++N;
	}
	// issueAbstract
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li>本文需要更多讲解， 便于帮助理解。</li>");
		++N;
	}
	// issueNeedCite
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li>本文需要更多参考文献。</li>");
		++N;
	}
	// issueAiRaw
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li> 本文含人工智能辅助创作，待审核。</li>");
		++N;
	}
	// issueAi
	...
	if (ind0 > 0) {
		ind1 = skip_command(str, ind0);
		str.replace(ind0, ind1 - ind0, u8"<li> 本文含人工智能辅助创作，已审核。</li>");
		++N;
	}
	// issueOther
	ind0 = 0;
